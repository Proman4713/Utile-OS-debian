...
3. `git remote add ubuntu https://git.launchpad.net/ubuntu/+source/{package}` to add the package's remote repository.
4. `git remote update ubuntu` to fetch the remote repository.
5. `git merge ubuntu/ubuntu/{codename} --allow-unrelated-histories` to merge the remote repository into the local repository.
6. (optional) if you did step 3., remove the `.git` folder from the package folder.
